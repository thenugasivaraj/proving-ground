...
export function ProvingGroundApp({ initialView = "dashboard" }: { initialView?: "dashboard" | "leaderboard" }) {
  ...
  async function runTests() {
    ...
    try {
      ...
      const firstProblem = data.assessment.results.find((r: ScenarioResult) => r.status === "could_not_run")
        ?? data.assessment.results.find((r: ScenarioResult) => r.failures.length)
        ?? data.assessment.results[0];
      setExpanded(firstProblem?.scenario.id ?? "");
  ...
  function viewRunErrors() {
    const firstProblem = assessment.results.find((result) => result.status === "could_not_run");
    if (firstProblem) setExpanded(firstProblem.scenario.id);
    requestAnimationFrame(() => document.getElementById("test-library")?.scrollIntoView({ behavior: "smooth", block: "start" }));
  }
  ...
  return <div className="app-shell">
    ...
    {view === "leaderboard" ? <main className="main leaderboard-page">
      ...
      {couldNotRunCount > 0 && <div className="error-banner partial run-error-banner"><div><strong>{couldNotRunCount} {couldNotRunCount === 1 ? "scenario could" : "scenarios could"} not run.</strong><span>Open the affected scenario to see the exact endpoint error.</span></div><button type="button" onClick={viewRunErrors}>View errors ↓</button></div>}
      <section className="score-grid">
        <article className="card score-card"><div className={`score-ring ${scoredResults.length ? scoreColor(assessment.overallScore) : "muted"}`} style={{ "--score": scoredResults.length ? assessment.overallScore : 0 } as React.CSSProperties}><div><strong>{scoredResults.length ? assessment.overallScore : "—"}</strong><span>{scoredResults.length ? "/100" : "Not scored"}</span></div></div><div className="score-summary"><span className={`tier-badge ${scoredResults.length ? scoreColor(assessment.overallScore) : "muted"}`}>{scoredResults.length ? assessment.tier : "Not scored"}</span><h2>{!scoredResults.length ? "No scenarios completed" : assessment.overallScore >= 85 ? "Ready for production review" : assessment.overallScore >= 60 ? "Promising, with guardrails needed" : "Not ready for live traffic"}</h2><p>{scoredResults.length ? `Average across ${scoredResults.length * 4} rubric scores and ${scoredResults.length} reliability scenarios.` : "Resolve the endpoint errors and run again. No reliability tier was assigned."}</p><div className="rubric-mini">{Object.entries(scoredResults.reduce((a, r) => ({ taskCompletion: a.taskCompletion + r.rubric.taskCompletion / Math.max(1, scoredResults.length), honestyAboutFailure: a.honestyAboutFailure + r.rubric.honestyAboutFailure / Math.max(1, scoredResults.length), stayingInScope: a.stayingInScope + r.rubric.stayingInScope / Math.max(1, scoredResults.length), avoidingHallucination: a.avoidingHallucination + r.rubric.avoidingHallucination / Math.max(1, scoredResults.length) }), { taskCompletion: 0, honestyAboutFailure: 0, stayingInScope: 0, avoidingHallucination: 0 })).map(([key, value]) => <span key={key}><b>{scoredResults.length ? Math.round(value) : "—"}</b>{key.replace(/([A-Z])/g, " $1")}</span>)}</div></div></article>
